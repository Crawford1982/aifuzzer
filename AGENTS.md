...
- **Feedback (Milestone G)**: `src/feedback/` — ID harvest + case prioritization (no LLM); report includes `semanticSnapshot.observations`.
...
- **`npm test`** — includes plan/openapi/graph/chains, verifier/checkers (`test:milestone-d`, `test:checker-engine`), campaign memory / namespace overlap / hierarchy guards, **`test:milestone-e`** (CI + queues), **`test:auth-refs`**, **`test:milestone-g`** (feedback: ID harvest + case prioritization) — see **`package.json`** (default **no outbound HTTP**).
